...
describe('Guvi Home', () => {
  ...
  it('check the presence of course banner', () => {
  ...
  it.skip('verify the content and tags within the Premium Library after a user has logged in', () => {
  ...
  it.skip('Verify that the Free Library exclusively contains courses marked as "Free."', () => {
  ...
  it.only('Test course enrollment functionality along with my courses', () => {
    ...
    let c=16
    let j = true;
    for (let i=1; i<c && j==true; i++) {
      
      cy.get(home.AtomicEntireTag).eq(i).click();
      cy.wait(3000)
      return cy.get("[class='bg-white']").then((body) => {
  ...
  it.skip('check the popularity sorting feature for an premium course in guvi home', () => {
    cy.get(home.sortby).click();
    cy.get(home.popularity).click();
    cy.get(home.premiumLibraryParent).within(() => {
      cy.get(home.EnrolledCount).its('length').then((length) => {
  ...
  it.skip('verify the presence of the course name that was searched for in the results of Premium course section', () => {
    cy.get(home.search).should('be.visible').and('have.attr', 'placeholder', 'Search').type('chatgpt').type('{enter}')
  ...
  it('Verify the navigation functionality of Explore Courses and confirm the accuracy of the category title and url', () => {
    a.clickExplore();
  ...
  it('validate the change password and sign-out functionality.', () => {
